...
static void on_write(ble_nus_t * p_nus, ble_evt_t * p_ble_evt)
{
    ...
    else
    {
		...
		if((p_evt_write->handle == p_nus->gyro_x_handles.cccd_handle)
        &&(p_evt_write->len == 2))
     {
        if (ble_srv_is_notification_enabled(p_evt_write->data))
        {
        ...
        else
        {
            p_nus->is_notification_enabled = false;
             ...
             (p_evt_write->handle == p_nus->gyro_x_handles.value_handle)
             &&(p_nus->data_handler != NULL))
     {
        p_nus->data_handler(p_nus, p_evt_write->data, p_evt_write->len);
     }
...
/**@brief       Function for adding GYRO_X characteristic.
 *
 * @param[in]   p_nus        Nordic UART Service structure.
 * @param[in]   p_nus_init   Information needed to initialize the service.
 *
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t gyro_x_char_add(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    /**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ...
    ble_uuid.type             = p_nus->uuid_type;
    ble_uuid.uuid             = BLE_UUID_NUS_GYRO_X_CHARACTERISTIC;
    ...
    attr_char_value.init_len  = sizeof(uint8_t);
    attr_char_value.init_offs = 0;
    attr_char_value.max_len   = BLE_NUS_MAX_GYRO_X_CHAR_LEN;
    
    return sd_ble_gatts_characteristic_add(p_nus->service_handle,
                                           &char_md,
                                           &attr_char_value,
                                           &p_nus->gyro_x_handles);
    /**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
}
...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    if (err_code != NRF_SUCCESS)
    {
		...
		// Add GYRO_X Characteristic.
    err_code = gyro_x_char_add(p_nus, p_nus_init);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_nus_send_gyro(ble_nus_t * p_nus, uint8_t * string, uint16_t length)
{
    ...
    hvx_params.handle = p_nus->gyro_x_handles.value_handle;
    hvx_params.p_data = string;
    hvx_params.p_len  = &length;
    hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
